...
소스열 **−3.8** → port **−3** → RDL **[−3,−1]** → 1차 보호(D_up/D_down) **−0.2**
...
- **RDL**: 좌측 port에 직결(몸체 왼쪽 정렬, [−3,−1]).
- **RDD_un1/dn1**: 좌측 tap 노드(victim rail tap, 5.1)에 직결. 우측 끝 = N3/N3B(clamp 단자).
- **Resd**: 연결 노드(IO −0.2 ↔ 2차 보호 2.6) 사이 **중앙**([0.2,2.2], center 1.2).
