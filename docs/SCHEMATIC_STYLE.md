...
(rail에 4-tap 병렬 금지). 치수: arm 간격 0.6(세로쌍)~1.05(가로쌍 y=±0.525),
...
4개 모두 **x=−3.8 단일 열** 수직 정렬, 각 스팬 3.0 (몸체 y=4.5/1.5/−1.5/−5.55):
- IO→VDD (3→6, 화살표 ↑ VDD)
- IO→VSS (0→3, 화살표 ↑ IO)
- GND→VSS (−3→0, 화살표 ↑ VSS) — **전용 ground** 를 하단(−3.8,−3)에 직결
- GND→MVSS (−7.05→−4.05, 화살표 ↑ MVSS) — **전용 ground** 를 하단(−3.8,−7.05)에 직결.
  상단은 (−3,−4.05) 경유로 MVSS port에 **아래에서 진입** (위 소스의 ground와 충돌 회피)
