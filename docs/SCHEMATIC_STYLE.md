...
세로 요소: VSS↔MVSS 수직 b2b(D_b2b_m) — tap (1.2,0)/(1.2,−2) 각 1개, 분기 y=−0.3/−1.7,
arm x=0.6/1.8 (Resd center 1.2와 수직 정렬, 1차·2차 보호 열 사이).

**b2b 쌍 공통 형태 — 2-port 묶음**: 각 net에 tap 1개만 두고
`tap → 짧은 stub → 분기 → 역병렬 arm 2개 → 병합 → stub → tap` 구조로 그린다
(rail에 4-tap 병렬 금지). 가로(도메인2 D_b2b)·세로(D_b2b_m) 모두 동일 패턴.
